...
nsims=1000 
sigma= [0.2, 0.3]           
corr_matrix = [[1 ,0.5],[0.5, 1]]
nexercise = 50 
#dummy for regression with higher power (=1 if higher powers, =0 if status quo)
high_pow=0

#########################################
## Simulate correlated stocks
####################################
def CorrelatedBrownian(S0,r,sigma,corr_matrix,T,steps,nsims):
    #rng default
    ...
    #
    # Output: S:              2 dimensional matrix of size steps X nsims X n_assets
    ...
    S = np.zeros(steps+1,nsims,n_assets)
...
#######################
# Pricing fct americanMin
########################

def BasketBermudanCall(S0, K, r, T, steps, nsims, sigma, corr_matrix, nexercise, high_pow)
    ...
    antithetic=0
    V = BasketBermudanCall(S0, K, r, T, steps, nsims, sigma, corr_matrix, nexercise,antithetic, high_pow);