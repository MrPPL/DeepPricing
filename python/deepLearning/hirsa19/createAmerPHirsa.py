...
import sys 
import os
sys.path.append(os.path.abspath("/home/ppl/Documents/Universitet/KUKandidat/Speciale/DeepHedging/python/BinomialModel"))
import torch
import BinoHull
...
def findY(X):
    ...
    for row in X:
        Y[count] =  float(BinoHull.findPayoff(steps=40, maturity=row[1], spot=row[0]*K, vol= row[3], strike = K, r=row[2])['0.0'][0])/K 
        count+=1
    return Y
...
df.to_csv("/home/ppl/Documents/Universitet/KUKandidat/Speciale/DeepHedging/python/deepLearning/hirsa19/data/mediumPAmerData.csv")
