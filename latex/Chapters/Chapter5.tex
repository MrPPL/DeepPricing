...
The American put on minimum of two assets option has almost double the number of parameters compared to the univariate contingent claims. Hence, the performance might produce a slightly higher MSE. For training we use a batch size of 64 based on earlier discussions. Table \ref{tab:AmerMinPerformanceComparision} and figure \ref{fig:MLPsInSampleAmerMin} show that the MLPs also fits the American put minimum option on two assets well. \\ 
